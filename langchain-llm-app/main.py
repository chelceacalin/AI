...
if submit_button:
    if len(youtube_url) < 10:
        ...
        # Get response and show it to user and give option to download the output
        db = lch.create_vector_db_from_url(youtube_url)
        response: str = lch.get_response_from_query(db = db, query = question_input,
                                                    output_language = language_selected, k = 5)
        st.write(response)
